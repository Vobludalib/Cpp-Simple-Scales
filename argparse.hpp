...
std::vector<std::string> inline split(const std::string& str)
{
    ...
    while (std::getline(ss, key, ','))
    {
        if (!key.empty() && key.back() == '\0')
            key.pop_back();  // last variables contain a '\0', which is unexpected when comparing to
                             // raw string, e.g. value == "test" will fail when the last character
                             // is '\0'. Therefore we can remove it
...
template <>
inline std::wstring get(const std::string& v)
{
...
inline char get(const std::string& v)
{
    return v.empty() ? throw std::invalid_argument("empty string")
           : v.size() > 1
               ? v.substr(0, 2) == "0x" ? (char)std::stoul(v, nullptr, 16) : (char)std::stoi(v)
               : v[0];
}
template <>
inline int get(const std::string& v)
{
    return std::stoi(v);
}
template <>
inline short get(const std::string& v)
{
    return std::stoi(v);
}
template <>
inline long get(const std::string& v)
{
    return std::stol(v);
}
template <>
inline long long get(const std::string& v)
{
    return std::stol(v);
}
template <>
...
template <>
inline double get(const std::string& v)
{
    return std::stod(v);
}
template <>
...
template <>
inline unsigned long long get(const std::string& v)
{
    return std::stoul(v);
}
...
{  // remaining types
    ...
    {
        return new
            typename std::remove_pointer<T>::type(get<typename std::remove_pointer<T>::type>(v));
    }
    ...
    {  // case-insensitive enum conversion
        ...
        throw std::runtime_error(
            "Enum not supported, please install magic_enum "
...
struct ConvertType : public ConvertBase
{
    ...
    {
        if (this->get_type_id() ==
            default_value->get_type_id())  // When the types do not match exactly. resort to string
                                           // conversion
...
struct Entry
{
    ...
    // Allow both string inputs and direct-type inputs. Where a string-input will be converted like
    // it would when using the commandline, and the direct approach is to simply use the value
    // provided.
    template <typename T>
    ...
    operator T()
    {
    }  // When you get here  because you received an error, make sure all parameters of argparse are
       // references (e.g. with `&`)
   ...
   private:
    ...
    void _convert(const std::string& value)
    {
        ...
        {
            error = "Invalid argument, could not convert \"" + value + "\" for " + _get_keys() +
                    " (" + help + ")";
        }
...
class Args
{
   private:
    ...
    bool has_options()
    {
        return std::find_if(all_entries.begin(), all_entries.end(),
                            [](auto e) { return e->type != Entry::ARG; }) != all_entries.end();
    };
   ...
   public:
    ...
    /* Add a positional argument, the order in which it is defined equals the order in which they
     * are being read. help : Description of the variable
     *
     * Returns a reference to the Entry, which will collapse into the requested type in
     * `Entry::operator T()`
     */
    Entry& arg(const std::string& help) { return arg("arg_" + std::to_string(_arg_idx), help); }

    /* Add a *named* positional argument, the order in which it is defined equals the order in which
     * they are being read. key : The name of the argument, otherwise arg_<position> will be used
    ...
    Entry& arg(const std::string& key, const std::string& help)
    {
        ...
        // Increasing _arg_idx, so that arg2 will be arg_2, irregardless of whether it is preceded
        // by other positional arguments
        _arg_idx++;
     ...
     * key : A comma-separated string, e.g. "k,key", which denotes the short (-k) and long(--key)
     * keys_ help : Description of the variable implicit_value : Implicit values are used when no
     * value is provided.
     *
     * Returns a reference to the Entry, which will collapse into the requested type in
     * `Entry::operator T()`
     ...
     * key : A comma-separated string, e.g. "k,key", which denotes the short (-k) and long(--key)
     * keys_ help : Description of the variable
     *
    ...
    /* Add a a subcommand
     * command : name of the subcommand, e.g. 'commit', if you wish to implement a function like
     * 'git commit'
     *
     * Returns a reference to the Entry, which will collapse into the requested type in
     * `Entry::operator T()` Expected type *Must* be an std::shared_ptr of derivative of the
     * argparse::Args class
     */
    ...
    virtual void help()
    {
        ...
        for (const auto& entry : arg_entries) cout << entry->keys_[0] << ' ';
        ...
        for (const auto& entry : all_entries)
        {
            ...
            {
                cout << setw(17) << entry->_get_keys() << " : " << entry->help << entry->info()
                     << endl;
            }
        }
     ...
     * Upon error, it will print the error and exit immediately if validation_action is
     * ValidationAction::EXIT_ON_ERROR
     */
    void parse(int argc, const char* const* argv, const bool& raise_on_error)
    {
        ...
        argc = parse_subcommands();  // argc_ is the number of arguments that should be parsed after
                                     // the subcommand has finished parsing
        ...
        auto is_value = [&](const size_t& i) -> bool
        {
            return params.size() > i &&
                   (params[i][0] != '-' ||
                    (params[i].size() > 1 &&
                     std::isdigit(params[i][1])));  // check for number to not accidentally mark
            ...
            if (itt != kwarg_entries.end())
            {
                ...
                {  // short values are not allowed to look ahead for the next parameter
                    if (is_value(i + 1))
                    {
                        ...
                        {
                            while (is_value(i + 1)) value += "," + params[++i];
                        }
        ...
        // Parse all the positional arguments, making sure multi_argument positional arguments are
        // processed last to enable arguments afterwards
        size_t arg_i = 0;
        ...
        {  // iterate over positional arguments until a multi-argument is found
            if (arg_i < arguments_flat.size()) arg_entries[arg_i]->_convert(arguments_flat[arg_i]);
        }
        size_t arg_j = 1;
        for (size_t j_end = arg_entries.size() - arg_i; arg_j <= j_end; arg_j++)
        {  // iterate from back to front, to ensure non-multi-arguments in the front and back are
           // given preference
            ...
            if (flat_idx < arguments_flat.size() && flat_idx >= arg_i)
            {
                ...
                {
                    std::stringstream
                        s;  // Combine multiple arguments into 1 comma-separated string for parsing
